...
以下のテンプレートに従う。CI（`validate.mts`）が構造を検査する。

#### announcement の必須深さ基準（CI 検査対象）

frontmatter チェック（タグ・トピック・ドメイン等）は **エラー**（commit をブロック）。本文深さチェックは **警告**（commit はブロックしないが CI ログに表示）。

| ルール | 閾値 | 検査方法 |
|--------|------|----------|
| 必須セクション | `## 概要`、`## 主な発表`、`## 詳細`、`## 応用シナリオ`、`## 参考リンク` が全て存在 | 見出し文字列の存在確認 |
| 「主な発表」の項目数 | ≥ 3 | `- **` で始まる行をカウント |
| 「詳細」のサブセクション数 | ≥ 3 | `### ` で始まる行をカウント |
| 「応用シナリオ」の項目数 | ≥ 3 | `- ` で始まる行をカウント |
| 「参考リンク」のリンク数 | ≥ 3 | `- [` で始まる行をカウント |
| 本文行数（frontmatter 除く） | ≥ 60 | 行数カウント |

`## 制約・注意点` と `## 関連エントリ` は任意セクション。内容がなければ省略可。
...
最低 3 件（CI 検査閾値）。5 件以上を推奨。以下のカテゴリで整理する。

- [公式ブログ記事](https://devblogs.microsoft.com/...)
- [ドキュメント](https://learn.microsoft.com/...)
- [クイックスタート](https://learn.microsoft.com/...)
- [Changelog](https://github.blog/changelog/...)
- [Build セッション](https://build.microsoft.com/sessions/...)

## 関連エントリ

content/ 内の関連エントリへの相互リンク。内容がなければセクションごと省略可。

- [関連エントリタイトル](../related-entry.md) — 関係の説明
```
...
| スピーカー | 名前（役職） |
| 日時 | YYYY年MM月DD日 HH:MM–HH:MM UTC |
| 会場 | 会場名 |
| 録画 | [オンデマンド](URL) |
| スライド | [PPTX](URL) |
...
セッションの流れに沿って技術的内容を構造化する。
スライドの要点、デモの実演内容、スピーカーの口頭補足、
コードサンプル、アーキテクチャ図の説明を統合して記載する。
プレゼンの進行順に `###` で分割し、セッションを視聴したのと
同等の情報をこのセクションから得られることを目指す。
トランスクリプトが利用可能な場合は要点を整理して織り込む。
...
- [関連 announcement](../announcements/{id}.md) — 関係の説明
```
