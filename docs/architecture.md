...
.github/                        ← GitHub 固有設定
├── agents/researcher.agent.md  ← 調査エージェント（スキルへのディスパッチャ）
├── skills/                     ← コンテンツ作成スキル
│   ├── research/SKILL.md       ← 調査方法論（共通基盤）
│   ├── announcement-summary/   ← announcements/*.md 作成
│   └── session-summary/        ← sessions/*.md 作成（スクリプト付き）
└── workflows/                  ← CI/CD

.cache/                         ← ローカルソースキャッシュ（gitignore 対象）
└── sessions/{session-code}/    ← セッション単位
    ├── meta.json               ← msevents CLI のメタデータ
    ├── transcript.vtt          ← 英語字幕（VTT 原本）
    ├── transcript.txt          ← VTT → プレーンテキスト変換
    ├── transcript.pdf          ← キーノート用トランスクリプト PDF
    ├── slides.pptx             ← スライドデッキ
    └── slides.md               ← スライドテキスト抽出結果
```
...
| `.github/skills/`           | コミット   | コンテンツ作成スキル             |
| `.cache/`                   | .gitignore | 著作権物のローカルキャッシュ（字幕・スライド等） |
